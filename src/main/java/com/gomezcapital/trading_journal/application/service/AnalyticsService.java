...
public class AnalyticsService {
    ...
    public AdvancedAnalyticsResponse calculateAdvancedAnalytics(UUID portfolioId) {
        ...
        for (Trade t : closedTrades) {
            ...
            // Excursiones (Calculamos la distancia absoluta entre la Entrada y el Pico)
            if (t.maePrice() != null && t.entryPrice() != null) { 
                totalMae = totalMae.add(t.entryPrice().subtract(t.maePrice()).abs()); 
                maeCount++; 
            }
            if (t.mfePrice() != null && t.entryPrice() != null) { 
                totalMfe = totalMfe.add(t.entryPrice().subtract(t.mfePrice()).abs()); 
                mfeCount++; 
            }
        }
